...
namespace CoreCms.Net.Repository
{
    ...
    public class CoreCmsReportsRepository : BaseRepository<GetOrdersReportsDbSelectOut>, ICoreCmsReportsRepository
    {
        ...
        {
            var sql = @"select top 1000 sum(oi.nums) as nums,sum(oi.amount) as amount,oi.sn,oi.name,oi.imageUrl,oi.addon 
            ...
            var page = await DbClient.SqlQueryable<GoodsSalesVolume>(sql).ToPageListAsync(pageIndex, pageSize, totalCount);
        ...
        {
            var sql = @"select top 1000 count(gc.goodsId) as nums,gc.goodsId,gc.goodsName,g.images 
            ...
            RefAsync<int> totalCount = 0;
            var page = await DbClient.SqlQueryable<GoodsCollection>(sql).ToPageListAsync(pageIndex, pageSize, totalCount);
