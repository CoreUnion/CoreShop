				...
				<view class="coreshop-content-top">
								...
								<label class="uni-list-cell uni-list-cell-pd">
									...
									<view class="img-list-item">
										...
										<view class="img-list-item-r">
											...
											<view class="goods-item-c u-margin-top-10 u-margin-bottom-10"
												v-if="item.addon">
											...
											<view class="goods-item-c">
												<view class="goods-buy">
													...
													<view class="goods-salesvolume u-margin-top-10 u-margin-bottom-10"
														v-if="!item.disabled">
														<label>您要退货数量：</label>
					...
					<view class='coreshop-cell-group u-margin-top-20 u-margin-bottom-20'>
								...
								<view class="uni-form-item uni-column invoice-type">
									...
									<radio-group class="uni-list" @change="radioChange">
										<label class="uni-list-cell uni-list-cell-pd" v-for="(item, index) in typeList"
											...
											<view class="invoice-type-icon">
												<radio class="a-radio radioNo" v-if="item.disabled" :id="item.name"
													:value="item.value" :checked="item.checked"
													:disabled="item.disabled"></radio>
												<radio class="a-radio " v-else :id="item.name" :value="item.value"
													:checked="item.checked" :disabled="item.disabled"></radio>
											</view>
									...
									<radio-group class="uni-list" @change="radioChange">
										<label class="uni-list-cell uni-list-cell-pd"
											...
											<view class="invoice-type-icon">
												<radio class="a-radio" :id="item.name" :value="item.value"
													:checked="item.checked" :disabled="item.disabled"></radio>
											</view>
											<view class="invoice-type-c">
						...
						<view class='coreshop-cell-item refund-price'>
							...
							<view class='coreshop-cell-item-ft'>
								<input type="digit" class='coreshop-cell-bd-input red-price' v-model="refund"
									@focus="refundFocus" ref="refund"></input>
							</view>
					...
					<view class="coreshop-bottomBox">
						<button class="coreshop-btn coreshop-btn-b coreshop-btn-square" formType="submit"
							:disabled='submitStatus' :loading='submitStatus'>提交</button>
					</view>
				</view>
			</form>
		</view>
			...
			getOrderInfo() {
				...
				this.$u.api.orderDetail(data).then(res => {
					if (res.status) {
						...
						if (res.data.globalStatus != 1 && res.data.globalStatus != 6 && res.data.globalStatus !=
							7) {
							//判断是已付款未发货，如果是，就禁用退货
							...
							this.refund = this.$common.formatMoney((res.data.orderAmount - res.data.refunded), 2,
								'');
							this.maxRefund = this.$common.formatMoney((res.data.orderAmount - res.data.refunded),
								2, '');
						...
						if (this.items[j].id == k) {
							if (this.items[j].nums >= this.items[j].reshipNums) {
								...
								if (nums >= this.items[j].returnNums) {
									nums = this.items[j].returnNums
									for (var s = 0; s < this.itemIds.length; s++) {
