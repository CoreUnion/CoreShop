<template>
    ...
    <view v-if="count">
        <view class="u-margin-left-20 u-margin-right-20 u-margin-top-20 u-margin-bottom-20">
            <u-section font-size="30" :title="corecmsdata.parameters.title" @click="goSeckillList()"></u-section>
        </view>
        <view class="u-margin-left-15  u-margin-right-15" v-if="corecmsdata.parameters.list && count">
            <view class="img-list-item" v-if="item.id !== 'undefined' && item.id" v-for="(item, key) in corecmsdata.parameters.list" :key="key" @click="goSeckillDetail(item.goods.id, item.goods.groupId)">
                ...
                <view class="img-list-item-r">
                    ...
                    <view class="item-c">
                        <view class="red-price">￥{{item.goods.product.price}} <span class="u-font-xs color-9 linethrough u-margin-left-15">{{item.goods.product.mktprice}}元</span></view>
                        <view>
            ...
            <view class="img-list-item" v-if="!item.id" v-for="(item, key) in corecmsdata.parameters.list" :key="key">
...
<style lang="scss" scoped>
    .img-list-item { border-radius: 8px; margin: 5rpx 5rpx 20rpx 5rpx; background-color: #ffffff; padding: 10rpx; position: relative; overflow: hidden; }
        .img-list-item .item-c { width: 100%; margin-top: 0; }
